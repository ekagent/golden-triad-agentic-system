...
const REGISTRY = [
  ...
  },
  {
    id: "mimo",
    label: "Mimo AI",
    description: "Primary direct LLM provider (fast, cost-effective).",
    envKeys: ["MIMO_API_KEY"],
    get configured() { return Boolean(process.env.MIMO_API_KEY); },
    get baseUrl() { return process.env.MIMO_BASE_URL || "https://api.mimo.ai/v1"; },
    get secretValue() { return process.env.MIMO_API_KEY; }
  },
  {
    id: "openrouter",
    label: "OpenRouter",
  ...
  },
  {
    id: "mcp",
    label: "MCP Server",
    description: "Custom Model Context Protocol server for extended tools.",
    envKeys: ["MCP_SERVER_URL"],
    get configured() { return Boolean(process.env.MCP_SERVER_URL); }
  }
];

export function getIntegrationConfig(id, userKeys = {}) {
