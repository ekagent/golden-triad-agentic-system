...
const MIN_PROVIDER_TIMEOUT_MS = 15000;
...
function isRetryableError(status, errorMsg) {
  ...
  if (/rate limit|quota|timeout|network|ECONNREFUSED|ENOTFOUND|ETIMEDOUT/i.test(errorMsg)) return true;
  return false;
}

async function waitForRateLimitRetry(attempt, maxRetries) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 429) {
            await waitForRateLimitRetry(retry, MAX_RETRIES);
          } else if (!isRetryableError(response.status, attemptError)) {
            break;
          }
