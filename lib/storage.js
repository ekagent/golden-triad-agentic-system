...
export async function addToWaitlist(email) {
  ...
  if (!client) return { status: "local-mode", detail: "Database not configured" };
...
function normalizeApiKeys(apiKeys = {}) {
  return Object.fromEntries(
    Object.entries(apiKeys)
      .map(([providerId, value]) => [providerId, normalizeSecretValue(value)])
...
export async function updateUserSettings(clerkUserId, settings) {
  const client = await getClient();
  const normalizedApiKeys = normalizeApiKeys(settings.api_keys || {});
  const normalizedRoleModels = settings.role_models || {};
  ...
  if (client) {
    await client`
      ...
      values (
        ${clerkUserId}, 
        ${client.json(normalizedApiKeys)}, 
        ${client.json(normalizedRoleModels)}
      )
      on conflict (clerk_user_id) do update
  ...
  allSettings[clerkUserId] = {
    ...
    role_models: normalizedRoleModels
  };
